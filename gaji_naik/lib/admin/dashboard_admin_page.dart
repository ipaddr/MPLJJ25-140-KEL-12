...
import 'package:gaji_naik/admin/manage_new_page.dart';
...
class _AdminDashboardPageState extends State<AdminDashboardPage> {
  ...
  int _totalBerita = 0; // Tambah statistik berita
  int _pendingReports = 0;
  int _activeEducationMaterials = 0;
  int _activeBerita = 0; // Tambah berita aktif
  ...
  void _loadCardData() async {
    try {
      ...
      await Future.wait([
        ...
        _loadEducationData(),
        _loadInstansiData(),
        _loadBeritaData(), // Tambah load berita
      ]);
  ...
  // Load Berita statistics untuk card
  Future<void> _loadBeritaData() async {
    try {
      QuerySnapshot beritaSnapshot =
          await _firestore.collection('berita').get();

      int total = beritaSnapshot.docs.length;
      int active = 0;
      ...
      setState(() {
        _totalBerita = total;
        _activeBerita = active;
      });
    } catch (e) {
      print('Error loading berita data: $e');
    }
  }
                    ...
                    children: <Widget>[
                      ...
                      GridView.count(
                        ...
                        children: <Widget>[
                          ...
                          _buildSummaryCard(
                            _isLoadingCards
                                ? '...'
                                : '$_totalBerita', // Update berita
                            'Total Berita',
                            Icons.newspaper,
                            Colors.red,
                            subtitle: _isLoadingCards
                                ? 'Loading...'
                                : '$_activeBerita berita aktif',
                          ),
                          _buildSummaryCard(
                            _isLoadingCards
                                ? '...'
                      ...
                      Column(
                        children: [
                          ...
                          const SizedBox(height: 16),
                          _buildMenuItem(
                            'Kelola Berita', // Menu baru
                            'Manage berita dan regulasi terbaru',
                            Icons.newspaper,
                            Colors.red,
                            () {
                              Navigator.push(
  ...
  // ...existing code... (sisa fungsi tetap sama)
