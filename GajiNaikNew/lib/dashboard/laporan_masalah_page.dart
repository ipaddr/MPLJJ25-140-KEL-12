...
class _LaporanMasalahPageState extends State<LaporanMasalahPage> {
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        if (userDoc.exists) {
          ...
          _showErrorMessage('Data profil tidak ditemukan. Silakan logout dan login kembali.');
        }
      } else {
  ...
  Future<void> _submitReport() async {
    ...
    if (_titleController.text.trim().isEmpty ||
      ...
      _showErrorMessage('Mohon lengkapi judul, deskripsi, dan kategori laporan');
      return;
    }

    // Validasi data profil (harus ada nama dan NIP minimal)
    if (_nameController.text.trim().isEmpty || _nipController.text.trim().isEmpty) {
      _showErrorMessage('Data profil tidak lengkap. Nama dan NIP harus terisi.');
      return;
    }
                            ...
                            Container(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  // Profile Info Cards
                                  _buildProfileInfoCard('Nama Lengkap', _nameController.text, Icons.person),
                                  const SizedBox(height: 12),
                                  _buildProfileInfoCard('NIP', _nipController.text, Icons.badge),
                                  const SizedBox(height: 12),
                                  _buildProfileInfoCard('No. Telepon', _phoneController.text, Icons.phone),
                                  if (_instansiController.text.isNotEmpty) ...[
                                    const SizedBox(height: 12),
                                    _buildProfileInfoCard('Instansi', _instansiController.text, Icons.business),
                                  ],
                                  if (_golonganController.text.isNotEmpty) ...[
                                    const SizedBox(height: 12),
                                    _buildProfileInfoCard('Golongan', _golonganController.text, Icons.star),
                                  ],
                                ],
                              ),
                            ),
