...
class _UserProfilePageState extends State<UserProfilePage> {
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        if (userDoc.exists) {
          ...
          print('Last login data: ${_userData['lastLoginAt']}');
          print('Created at data: ${_userData['createdAt']}');
          print('All user data: $_userData');
        } else {
  ...
  Future<void> _logout() async {
    try {
      // Update last logout timestamp before signing out
  ...
  // Format date from Timestamp - improved version
  ...
  // Get last login info - improved version
  String get _lastLoginInfo {
    // Check multiple possible field names for last login
    ...
    for (String field in possibleFields) {
      if (_userData[field] != null) {
        print(
            'Found last login in field: $field with value: ${_userData[field]}');
        return _formatDateTime(_userData[field]);
      }
    }

    // If no login timestamp found, check Firebase Auth user metadata
    User? currentUser = _auth.currentUser;
    if (currentUser?.metadata.lastSignInTime != null) {
      print(
          'Using Firebase Auth lastSignInTime: ${currentUser!.metadata.lastSignInTime}');
      return _formatDateTime(
          Timestamp.fromDate(currentUser.metadata.lastSignInTime!));
    }

    print('No last login data found in any field');
    return 'Tidak diketahui';
  }

  // Get account created info - improved version
  String get _accountCreatedInfo {
    // Check multiple possible field names for account creation
    final possibleFields = [
      'createdAt',
      'created_at',
      'createTime',
    ...
    for (String field in possibleFields) {
      if (_userData[field] != null) {
        print(
            'Found created date in field: $field with value: ${_userData[field]}');
        return _formatDate(_userData[field]);
      }
    }

    // If no creation timestamp found, check Firebase Auth user metadata
    User? currentUser = _auth.currentUser;
    if (currentUser?.metadata.creationTime != null) {
      print(
          'Using Firebase Auth creationTime: ${currentUser!.metadata.creationTime}');
      return _formatDate(
          Timestamp.fromDate(currentUser.metadata.creationTime!));
    }

    print('No creation date found in any field');
    return 'Tidak diketahui';
  }
                            ...
                            Container(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  // Quick stats - improved with better data handling
                                  Container(
